#ifndef _CIRCUALR_BUFFER_H
#define _CIRCUALR_BUFFER_H
...
class CircularBuffer
{
	...
	inline void AllocateB() { m_regionBPointer = m_buffer; }
...
public:
	...
	*/
	size_t GetSize() { return m_regionASize + m_regionBSize; }

	/** Returns the number of contiguous bytes (that can be pushed out in one operation)
	*/
	size_t GetContiguiousBytes() { // A before B
	...
	*/
	void * GetBuffer() {
		return m_regionBPointer ? &m_regionBPointer[m_regionBSize] : &m_regionAPointer[m_regionASize];
	}
