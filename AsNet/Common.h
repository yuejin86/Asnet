...
/*
#define REPACK "Titan's Repack"
#define REPACK_AUTHOR "Chaos"
...
/*FLAME_INLINE static void swap16(uint16* p) { *p = ((*p >> 8) & 0xff) | (*p << 8); }
FLAME_INLINE static void swap32(uint32* p) { *p = ((*p >> 24 & 0xff)) | ((*p >> 8) & 0xff00) | ((*p << 8) & 0xff0000) | (*p << 24); }
FLAME_INLINE static void swap64(uint64* p) { *p = ((*p >> 56)) | ((*p >> 40) & 0x000000000000ff00ULL) | ((*p >> 24) & 0x0000000000ff0000ULL) | ((*p >> 8 ) & 0x00000000ff000000ULL) |
								((*p << 8 ) & 0x000000ff00000000ULL) | ((*p << 24) & 0x0000ff0000000000ULL) | ((*p << 40) & 0x00ff000000000000ULL) | ((*p << 56)); }*/

FLAME_INLINE static void swap16(uint16* p) { *p = bswap_16((uint16_t)*p); }
FLAME_INLINE static void swap32(uint32* p) { *p = bswap_32((uint32_t)*p); }
FLAME_INLINE static void swap64(uint64* p) { *p = bswap_64((uint64_t)*p);; }

FLAME_INLINE static float swapfloat(float p)
{
...
FLAME_INLINE static double swapdouble(double p)
{
...
FLAME_INLINE static void swapfloat(float * p)
{
	union { float asfloat; uint8 asbytes[4]; } u1, u2;
	u1.asfloat = *p;
	/* swap! */
...
FLAME_INLINE static void swapdouble(double * p)
{
	union { double asfloat; uint8 asbytes[8]; } u1, u2;
	u1.asfloat = *p;
	/* swap! */
...
/*FLAME_INLINE static uint16 swap16(uint16 p) { return ((p >> 8) & 0xff) | (p << 8); }
FLAME_INLINE static uint32 swap32(uint32 p) { return ((p >> 24) & 0xff) | ((p >> 8) & 0xff00) | ((p << 8) & 0xff0000) | (p << 24); }
FLAME_INLINE static uint64 swap64(uint64 p)  { p = (((p >> 56) & 0xff)) | ((p >> 40) & 0x000000000000ff00ULL) | ((p >> 24) & 0x0000000000ff0000ULL) | ((p >> 8 ) & 0x00000000ff000000ULL) |
								((p << 8 ) & 0x000000ff00000000ULL) | ((p << 24) & 0x0000ff0000000000ULL) | ((p << 40) & 0x00ff000000000000ULL) | ((p << 56)); }

FLAME_INLINE static void swap16(int16* p) { *p = ((*p >> 8) & 0xff) | (*p << 8); }
FLAME_INLINE static void swap32(int32* p) { *p = ((*p >> 24) & 0xff) | ((*p >> 8) & 0xff00) | ((*p << 8) & 0xff0000) | (*p << 24); }
FLAME_INLINE static void swap64(int64* p) { *p = ((*p >> 56) & 0xff) | ((*p >> 40) & 0x000000000000ff00ULL) | ((*p >> 24) & 0x0000000000ff0000ULL) | ((*p >> 8 ) & 0x00000000ff000000ULL) |
								((*p << 8 ) & 0x000000ff00000000ULL) | ((*p << 24) & 0x0000ff0000000000ULL) | ((*p << 40) & 0x00ff000000000000ULL) | ((*p << 56)); }

FLAME_INLINE static int16 swap16(int16 p) { return ((p >> 8) & 0xff) | (p << 8); }
FLAME_INLINE static int32 swap32(int32 p) { return ((p >> 24) & 0xff) | ((p >> 8) & 0xff00) | ((p << 8) & 0xff0000) | (p << 24); }
FLAME_INLINE static int64 swap64(int64 p)  { return ((((p >> 56) & 0xff)) | ((p >> 40) & 0x000000000000ff00ULL) | ((p >> 24) & 0x0000000000ff0000ULL) | ((p >> 8 ) & 0x00000000ff000000ULL) |
								((p << 8 ) & 0x000000ff00000000ULL) | ((p << 24) & 0x0000ff0000000000ULL) | ((p << 40) & 0x00ff000000000000ULL) | ((p << 56))); }*/

FLAME_INLINE static uint16 swap16(uint16 p) { return bswap_16((uint16_t)p); }
FLAME_INLINE static uint32 swap32(uint32 p) { return bswap_32((uint32_t)p); }
FLAME_INLINE static uint64 swap64(uint64 p)  { return bswap_64((uint64_t)p); }

FLAME_INLINE static void swap16(int16* p) { *p = bswap_16((uint16_t)*p); }
FLAME_INLINE static void swap32(int32* p) { *p = bswap_32((uint32_t)*p); }
FLAME_INLINE static void swap64(int64* p) { *p = bswap_64((uint64_t)*p); }

FLAME_INLINE static int16 swap16(int16 p) { return bswap_16((uint16_t)p); }
FLAME_INLINE static int32 swap32(int32 p) { return bswap_32((uint32_t)p); }
FLAME_INLINE static int64 swap64(int64 p)  { return bswap_64((uint64_t)p); }
...
FLAME_INLINE void reverse_array(uint8 * pointer, size_t count)
{
	size_t x;
...
FLAME_INLINE void FLAME_TOLOWER(std::string& str)
{
	for(size_t i = 0; i < str.length(); ++i)
		str[i] = (char)tolower(str[i]);
};

FLAME_INLINE void FLAME_TOUPPER(std::string& str)
{
	for(size_t i = 0; i < str.length(); ++i)
...
static unsigned int MakeIP(const char * str)
{
	...
	unsigned int res;
	if( sscanf_s(str, "%u.%u.%u.%u", &bytes[0], &bytes[1], &bytes[2], &bytes[3]) != 4 )
		return 0;
...
static const char* _StringToUTF8(const char*   pASCIIBuf)
{
#ifdef WIN32
...
static const char* _StringToANSI(const char*   pUtf8Buf)
{
#ifdef WIN32
...
static bool _IsStringUTF8(const char* str)
{
	int   i;
...
static void get_start_end_point(uint32& p_start, uint32& p_end, uint32 p_radius)
{
	if (p_start > p_end)
		p_start = p_end;

	if (p_start + p_radius < p_end)
		p_end = p_start + p_radius;
